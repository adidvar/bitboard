        ...
        if constexpr ((flags & BitBoard::flags_el_passant)) {
              ...
              && enabled)
          {
            bitboard new_blockers =
                (all & (~attack_cell) & (~pawnsShift<-9>(to_mask))) | to_mask;
          ...
          {
            bitboard new_blockers =
                (all & (~attack_cell) & (~pawnsShift<-7>(to_mask))) | to_mask;
            if (!isMate(new_blockers)) {
...
template<BitBoard::Flags flags>
constexpr int generateTemplate(const BitBoard& board,
                               Turn* storage,
                               bool& in_check)
{
...
constexpr int generate(const BitBoard& board,
                       Turn* storage,
