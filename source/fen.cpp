...
void skipSeperators(std ::string_view data, size_t& index)
{
  while (index < data.size()
         && std::find(seperators.begin(), seperators.end(), data[index])
             != seperators.end())
  {
    index++;
  }
...
std::string boardToFen(const BitBoard& board)
{
  ...
  string += ' ';
  if ((rooking_flags
       & (BitBoard::flags_black_oo | BitBoard::flags_black_ooo
          | BitBoard::flags_white_oo | BitBoard::flags_white_ooo))
      == 0)
  {
    string.push_back('-');
  } else {
  ...
  if (rooking_flags & BitBoard::flags_el_passant) {
    string +=
        Position((board.getTurn().from().index() + board.getTurn().to().index())
                 / 2)
            .toString();
  } else {
    string += '-';
